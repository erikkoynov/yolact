...
class Log:
    ...
    def __init__(
        ...
        if log_gpu_stats and not nvsmi_available():
            print(
                "Warning: Log created with log_gpu_stats=True, but nvidia-smi "
...
class LogVisualizer:
    ...
    def sessions(self, path: str):
        ...
        def pop_session():
            ...
            print(
                "Session % 3d: % 8d entries | %s elapsed"
                % (cur_session, num_entries, time_str)
            )
    ...
    def bar(
        ...
        data_values = [
            [(datum[k] if k in datum else None) for k in data_labels]
            for datum in data_points
        ]
        ...
        # Make the plot
        for idx, (series, data, pos) in enumerate(
            zip(series_labels, data_values, positions)
        ):
            plt.bar(
                pos,
                data,
    ...
    def elapsed_time(
        self, cond1: str = "", cond2: str = "", legible: bool = True
    ) -> list:
        """
        ...
        diff = (
            (lambda x: str(datetime.timedelta(seconds=x)).split(".")[0])
            if legible
            else lambda x: x
        )
