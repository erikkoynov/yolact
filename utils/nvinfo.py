...
def gpu_info() -> list:
    ...
    for i in range(gpu_count):
        mem_used, mem_total = [
            ...
            for m in selected_lines[3 * i + 1].split("|")[2].strip().split("/")
        ]

        pw_tmp_info, mem_info, util_info = [
            x.strip() for x in selected_lines[3 * i + 1].split("|")[1:-1]
        ]

        pw_tmp_info = [x[:-1] for x in pw_tmp_info.split(" ") if len(x) > 0]
        fan_speed, temperature, pwr_used, pwr_cap = [
            int(pw_tmp_info[i]) for i in (0, 1, 3, 5)
        ]
